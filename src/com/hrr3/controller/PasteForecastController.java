...
public class PasteForecastController extends SelectorComposer<Component>  {
	private static final long serialVersionUID = 1L;
	
	protected Component mainApp;
	@Wire
    Window pasteForecastDialog;
	@Wire
	...
	public PasteForecastController() {
    ...
    public void pasteModelWeekSubmit(Event e) {
		...
		mainApp = this.pasteForecastDialog;		 	
		Clients.showBusy(mainApp,"Executing Paste Forecast, please wait...");
		mainApp.addEventListener(Events.ON_CLIENT_INFO, new EventListener<Event>() {
	        ...
	        public void onEvent(Event event) throws Exception {
	        	...
	        	trendDAO.pasteForecast(currentTrendToolData.getLstModelWeek(), sdf.format(fromPasteDate.getValue()), sdf.format(toPasteDate.getValue()), "ModelWeek");
	        	trendDAO.reCalculateAll(sdf.format(fromPasteDate.getValue()), sdf.format(toPasteDate.getValue()), currentCustomer.getCustomerId());
	    		Messagebox.show("Paste Model Week was succesfully executed","Paste Forecast", Messagebox.OK, null);
    ...
    public void applyTrendSubmit(Event e) {
		...
		mainApp.addEventListener(Events.ON_CLIENT_INFO, new EventListener<Event>() {
	        ...
	        public void onEvent(Event event) throws Exception {
	        	...
	        	DateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");   	
	    		trendDAO.pasteForecast(currentTrendToolData.getLstForecastTrend(), sdf.format(fromPasteDate.getValue()), sdf.format(toPasteDate.getValue()), "ForecastTrend");
	    		trendDAO.reCalculateAll(sdf.format(fromPasteDate.getValue()), sdf.format(toPasteDate.getValue()), currentCustomer.getCustomerId());
	    		Messagebox.show("Apply Trend was succesfully executed","Paste Forecast", Messagebox.OK, null);
